...
function buildBlocksFromPublication(publication) {
  if (publication.type === 'menu') {
    return publication.menu_jours.map((day) => ({
      type: day.isSpecial ? 'callout' : 'section',
      heading: day.day,
      body: day.isSpecial ? day.message : '',
      items: day.isSpecial ? [] : day.items.map((item) => serializeMenuItem(item)),
    }));
  }
...
export function buildCanonicalContent(rawContent, { fallbackId } = {}) {
  ...
  return {
    ...
    plainText: publication.type === 'menu' ? '' : publication.contenu_texte,
    menuDays: publication.type === 'menu' ? publication.menu_jours : [],
