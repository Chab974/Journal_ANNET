const ALLOWED_POST_TYPES = ['menu', 'alerte', 'evenement', 'info', 'coup_de_coeur'];
...
function normalizeMenuItem(item, dayLabel, index) {
  if (!isObject(item)) {
    return null;
  }
...
function normalizeMenuDay(day, index) {
  if (!isObject(day)) {
    return null;
  }
  ...
  const items = rawItems
    .map((item, itemIndex) => normalizeMenuItem(item, label, itemIndex))
    .filter(Boolean);
...
export function normalizePublication(rawPublication, { fallbackId } = {}) {
  ...
  const rawType = slugify(rawPublication.type);
  const type = ALLOWED_POST_TYPES.includes(rawType) ? rawType : 'info';
  ...
  if (publication.type === 'menu') {
    const rawDays = Array.isArray(rawPublication.menu_jours) ? rawPublication.menu_jours : [];
    publication.menu_jours = rawDays
      .map((day, index) => normalizeMenuDay(day, index))
      .filter(Boolean);

    if (publication.menu_jours.length === 0) {
      throw new Error('Publication invalide : menu_jours doit contenir au moins un jour exploitable.');
    }
  }
  ...
  if (publication.type !== 'menu' && !publication.contenu_texte) {
...
5. Si c'est un menu, remplis "menu_jours" avec des jours, items, descriptions et badges.
